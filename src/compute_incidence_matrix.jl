...
function incidence_matrix{S<:Integer,T<:Integer}(l1::Array{S}, l2::Array{T}, mask)
	incidence_dict=DefaultDict{Tuple{S,T},Int}(0)
	for i in eachindex(machine_labels, human_labels, mask)
		incidence_dict[(machine_labels[i],human_labels[i])]+=mask[i]
	end
	I=Int[]
	J=Int[]
	V=Int[]
