...
#returns the merge dendrogram as a MergeTree datastructure.
function forward2{T,S}(affinities::Array{T,4}, machine_labels::Array{S,3}; agglomerator=Agglomerators.MeanAffinityAgglomerator(), threshold=0.1, human_labels=nothing)
	rg, vertices, edges = compute_regiongraph(machine_labels, affinities)
	...
	mergetree = MergeTrees.to_merge_tree(rg,agglomerator)
	dump(mergetree)
	return mergetree
end
