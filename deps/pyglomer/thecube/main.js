...
var cache_domain = 'http://cache.eyewire.org'
cache_domain = 'http://localhost:8888'
...
function getStartingTiles(realTileNum, bundleSize, volId, chunk, axis, type, callback) {
  ...
  var url = cache_domain + "/volume/" + volId + "/chunk/0/" + chunk[0] + "/" + chunk[1] + "/" + chunkZ + "/tile/" + axis + "/" + range[0] + ":" + range[1];
...
function getImagesForVolXY(volId, chunk, axis, type, callback) {
  var url = cache_domain + "/volume/" + volId + "/chunk/0/" + chunk[0] + "/" + chunk[1] + "/" + chunk[2] + "/tile/" + axis + "/" + 0 + ":" + CHUNK_SIZE;
  $.get(url).done(function (tilesRes) {
    for (var trIdx = 0; trIdx < tilesRes.length; trIdx++) {
